...
def make_plot(args):
    ...
    x = args["rsun"]
    slopesun = v0 * (
        (1 / h) * np.power(1 + (x / h) ** 2, (p - 2) / 4)
        + (x**2 / h**3) * ((p - 2) / 2) * np.power(1 + (x / h) ** 2, (p - 6) / 4)
    )
    ...
    if not args["dr3story"]:
        ...
        ax.text(
            0.95,
            0.12,
            rf"$V_{{\mathrm{{circ}},\odot}} = {args['vcsun']:.1f}$ km s$^{{-1}}$",
            transform=ax.transAxes,
            ha="right",
            fontsize=12,
        )
        ax.text(
            0.95,
            0.05,
            rf"$dV_\mathrm{{circ}}/dR$ at sun: {slopesun:.1f} km s$^{{-1}}$ kpc$^{{-1}}$",
            transform=ax.transAxes,
            ha="right",
            fontsize=12,
        )
...
def parseCommandLineArguments():
    ...
    parser = argparse.ArgumentParser(
        description="""Rotation curve from Brunetti and Pfenniger (2010)."""
    )
    parser.add_argument(
        "--pexp",
        dest="pexp",
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="""Potential scale length (kpc)""",
    )
    parser.add_argument(
        "--vcsun",
        dest="vcsun",
        type=float,
        default=234.0,
        help="""Circular velocity for the sun (km/s)""",
    )
    parser.add_argument(
        "--rsun",
        dest="rsun",
        type=float,
        default=8.277,
        help="""Distance sun to Galactic centre (kpc)""",
    )
    parser.add_argument(
        "--dr3story",
        action="store_true",
        dest="dr3story",
        help="""Produce a version for the Gaia DR3 story on Milky Way rotation""",
    )
    parser.add_argument(
