"""
Functions for loading the Gaia DR3 samples from FITS files downloaded from the archive. These are samples intended for analysis of the Milky Way disk rotation curve and residual kinematics. The following are calculated from the input data:
...
Anthony Brown Jul 2022 - Dec 2022
"""
...
def correct_radvel_uncertainty(dr3_radvel_unc, dr3_grvs):
    ...
    faint = dr3_grvs > 12.0
    infl_factor[bright] = (
        0.318 + 0.3884 * dr3_grvs[bright] - 0.02778 * dr3_grvs[bright] ** 2
    )
    infl_factor[faint] = (
        16.554 - 2.4899 * dr3_grvs[faint] + 0.09933 * dr3_grvs[faint] ** 2
    )
...
def correct_radvel(dr3_radvel, dr3_template_teff, dr3_grvs):
    ...
    hot = (
        (dr3_template_teff >= 8500.0)
        & (dr3_template_teff <= 14500.0)
        & (dr3_grvs >= 6.0)
        & (dr3_grvs <= 12.0)
    )
    ...
    radvel_correct[cool] = (
        radvel_correct[cool]
        + 0.02755 * dr3_grvs[cool] ** 2
        - 0.55863 * dr3_grvs[cool]
        + 2.81129
    )
...
def load_mwtable(
    ...
    zsun=_zsun,
    sunpos=_sunpos,
    vsunpeculiar=_vsunpeculiar,
    vcircsun=_vcircsun,
):
    """
    Load the table. The table is expected to be in FITS format and the following Gaia DR3 data are expected to be present:

    gaiadr3.gaia_source: ra, dec, parallax, pmra, pmdec, radial_velocity, parallax_over_error, pmra_error, pmdec_error, pmra_pmdec_corr, radial_velocity_error, grvs_mag, rv_template_teff, phot_g_mean_mag, bp_rp
    ...
    gaiatable["parallax_over_error"] = (
        gaiatable["parallax"] / gaiatable["parallax_error"]
    )
    gaiatable["vtan"] = (
        (
            au_km_year_per_sec
            / gaiatable["parallax"]
            * np.sqrt(gaiatable["pmra"] ** 2 + gaiatable["pmdec"] ** 2)
        ).value
        * u.km
        / u.s
    )
    gaiatable["radial_velocity_corrected"] = gaiatable["radial_velocity"]
    gaiatable["radial_velocity_corrected"][gaiatable["rvvalid"]] = correct_radvel(
        gaiatable["radial_velocity"][gaiatable["rvvalid"]],
        gaiatable["rv_template_teff"][gaiatable["rvvalid"]],
        gaiatable["grvs_mag"][gaiatable["rvvalid"]],
    )
    gaiatable["radial_velocity_error_corrected"] = gaiatable["radial_velocity_error"]
    gaiatable["radial_velocity_error_corrected"][
        gaiatable["rvvalid"]
    ] = correct_radvel_uncertainty(
        gaiatable["radial_velocity_error"][gaiatable["rvvalid"]],
        gaiatable["grvs_mag"][gaiatable["rvvalid"]],
    )
    ...
    (
        galactic_coords,
        galactocentric_cartesian,
        galactocentric_cylindrical,
    ) = transform_to_galactic(
        icrs_coords,
        galcendist=Rsun,
        sunheight=zsun,
        vcircsun=vcircsun,
    ...
    (
        gaiatable["pml_error"],
        gaiatable["pmb_error"],
        gaiatable["pml_pmb_corr"],
    ) = ct.transform_proper_motion_errors(
        np.deg2rad(gaiatable["ra"]),
        np.deg2rad(gaiatable["dec"]),
        gaiatable["pmra_error"],
        gaiatable["pmdec_error"],
        rho_muphi_mutheta=gaiatable["pmra_pmdec_corr"],
    )
    ...
    #
    gaiatable["v_phi_gc"] = -(
        galactocentric_cylindrical.d_phi.to(u.rad / u.yr)
        / u.rad
        * galactocentric_cylindrical.rho
    ).to(u.km / u.s)
    gaiatable["vtot_lsr"] = np.sqrt(
        gaiatable["v_R_gc"] ** 2
        + (gaiatable["v_phi_gc"] - vcircsun.value) ** 2
        + gaiatable["v_z_gc"] ** 2
    )
    ...
    gaiatable["mg_abs0_gspphot"] = (
        gaiatable["gmag0_gspphot"] + 5 * np.log10(gaiatable["parallax"]) - 10
    )
    if esphs:
        ...
        gaiatable["mg_abs0_esphs"] = (
            gaiatable["gmag0_esphs"] + 5 * np.log10(gaiatable["parallax"]) - 10
        )
